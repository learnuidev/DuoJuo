(ns duolingo.recipe-tests
  (:require [clojure.test :refer :all]
            [duolingo.server :refer :all]
            [duolingo.test-system :as ts]))

(defn recipe-fixture
  [f]
  (ts/create-auth0-test-user
    {:connection "Username-Password-Authentication"
     :email      "recipe-tests@novus.app"
     :password   "s#m3R4nd0m-pass"})
  (reset! ts/token (ts/get-test-token "recipe-tests@novus.app"))
...
(deftest recipe-tests
  (testing "Create recipe"
    (let [{:keys [status body]} (ts/test-endpoint :post "/v1/recipes" {:auth true :body recipe})]
      (reset! recipe-id (:recipe_id body))
      (is (= status 201))))
